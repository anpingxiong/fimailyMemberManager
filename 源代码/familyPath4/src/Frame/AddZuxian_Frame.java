...
import Dao4MySqlImpl.MemberDaoImpl;
 ...
 public class AddZuxian_Frame extends javax.swing.JFrame implements FocusListener {

     public AddZuxian_Frame() {
    	 setLocation(500, 300);
        initComponents();  
   ...
   private void initComponents() {
        ...
        TextField_peiou.setText("如果未婚不需要填写");
        TextField_peiou.addFocusListener(new FocusAdapter() {
		 public void focusGained(FocusEvent e) {
		 	super.focusGained(e);
		 	TextField_peiou.selectAll();
		  }
        });
       ...
       jLabel5.setText("出生日期");
        ...
        RadioButton_man.setSelected(true);
        RadioButton_man.addActionListener(new ActionListener() {
		 	public void actionPerformed(ActionEvent e) {
		 		jLabel6.setText("妻子");
			}
		});
        RadioButton_woman.setText("女");
        RadioButton_woman.addActionListener(new ActionListener() {
		 	public void actionPerformed(ActionEvent e) {
		 		jLabel6.setText("丈夫");
			}
		});
        ...
        Button_up.addActionListener(new ActionListener() {
			 public void actionPerformed(ActionEvent e) {
			     ...
			     if(!AnfunctionImpl.checkDateIsNotError(TextField_year.getText().trim()+"-"+TextField_month.getText().trim()+""+TextField_day.getText().trim())
			      ...
			      Date  da=null;
					...
					MemberDaoImpl a = new MemberDaoImpl();
	             	...
	             	if(!((TextField_peiou.getText().trim()+"").equals("") || ((TextField_peiou.getText().trim()+"").equals("如果未婚不需要填写")) ))
				  {
	            
            		  pe  =  a.getMemberByName(TextField_peiou.getText().trim(), MainFrame.getFamily().getId());
      			 	 
            	  }
					 
					    //检测配偶					 
	             	if(!(TextField_peiou.getText().trim()+"").equals("如果未婚不需要填写"))
	             	{
	             		if(pe==null  ){
				    	JOptionPane.showMessageDialog(null,TextField_peiou.getText()+"不存在,请检查重新输入");
				    	return  ;
				    } else  if(pe.getSex().equals(sex)){
				    	JOptionPane.showMessageDialog(null,"拒绝同性结婚,请修改性别");
				    	return  ;
				    } else  if(pe.getAge()!=1){
				    	JOptionPane.showMessageDialog(null,pe.getName()+"不是祖先(第一代成员)");
				    	return  ;
				    }
                    if(pe.getPeiou()!=null){
                     	JOptionPane.showMessageDialog(null,pe.getName()+"有配偶,拒绝再次添加,请重新输入");
				    	return  ;
				     }
                    }
				...
				MemberDaoImpl  memberDao  =   new MemberDaoImpl();
					Member   aa  = memberDao.getMemberByName(na, MainFrame.getFamily().getId());
			 if(pe!=null){
			 pe.setPeiou(aa);
			 	memberDao.updateMember(pe, pe.getId());
			  }
    ...
    JLabel lb = new JLabel(new ImageIcon("./image/accept.png"));
    JLabel lb1 = new JLabel(new ImageIcon("./image/exclamation.png"));
